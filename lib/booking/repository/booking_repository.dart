...
class BookingRepository {
  static Future<List<Booking>> getCurrentBookings(username, password) async {
    final response = await http.get(
        '${DotEnv().env['SPORTEMPLE_API']}/booking/current-reservations',
        headers: {
          'club_id': '57920066',
          'username': username,
          'password': password,
  ...
  static Future<bool> cancel(
      String username, String password, String bookingId) async {
    final response = await http
        .post('${DotEnv().env['SPORTEMPLE_API']}/booking/cancel', body: {
